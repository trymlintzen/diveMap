...
class DiveMapService {
    ...
    func searchDiveMap(searchString: String)  {
        
        let urlString =  "\(urls.baseURL)?mode=search&str=\(searchString)"
        Alamofire.request( urlString ,
                           method: .get,
                           parameters: nil,
                           encoding: JSONEncoding.default).responseJSON { (jsonData) in
                            if let result = jsonData.result.value as? NSDictionary {
                                self.parseSearchData(result: result)
                            }
        }
    ...
    func parseSearchData (result: NSDictionary) {
        var diveItems: [DiveMapItems] = []
        
        if let sites = result["matches"] as? NSArray {
            for currentHit in sites {
                if let dict = currentHit as? NSDictionary,
                    let lat = dict["lat"] as? String,
                    let lng = dict["lng"] as? String,
        ...
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: notificationIDs.tableResultID),
                                        object: self,
                                        userInfo: [dictKey.diveURLKey : diveItems])
    }
    
    
    
//    func looping(key: String, dictionary: NSDictionary, object: DiveMapItems ) {
//        if let sites = dictionary["\(key)"] as? NSArray {
//            for currentHit in sites {
//                if let dict = currentHit as? NSDictionary,
//                    let lat = dict["lat"] as? String,
//                    let lng = dict["lng"] as? String,
